...
(defn- get-session
  ...
  [repository request]
  (if-let [session-data (-> request :cookies :skyway-session)]
...
(defn set-session-cookie
  ...
  (let [new?   (:new-session? request)
        ...
        update (set-cookie :skyway-session cookie
